...
/*   Updated: 2024/11/13 16:55:30 by ggaribot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    execute_external_command(t_cmd *cmd, t_env **env)
{
    ...
    if (!command_path)
    {
        ...
        cmd->exit_status = 127;  // Command not found
        return;
    }
    ...
    if (execve(command_path, cmd->args, env_array) == -1)
    {
        ...
        if (errno == ENOENT)
            ...
            cmd->exit_status = 126;  // Permission denied
        else
            cmd->exit_status = 1;    // General error
...
void    execute_command(t_cmd *cmd, t_env **env)
{
    ...
    // Update the environment's last exit status
